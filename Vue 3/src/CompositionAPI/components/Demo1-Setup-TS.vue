...
<script lang="ts">
import { ref, computed, onMounted, onUnmounted, defineComponent, onUpdated } from 'vue'
  ...
  setup() {
    ...
    function init(): void {
      const main = () => {
        ...
        const xStep: number = Math.abs(mousePos.value.x - x) / 60
        const yStep: number = Math.abs(mousePos.value.y - y) / 60
