...
@Component
export default class Demo1TSClass extends Vue {
  ballSize: number = 200
  ballPos: Point = { x: 0, y: 0 }
  mousePos: Point = { x: 0, y: 0 }

  get ballStyle(): string {
    let style = []
  ...
  init(): void {
    const main = () => {
      let x: number = this.ballPos.x,
        y: number = this.ballPos.y

      const xStep: number = Math.abs(this.mousePos.x - x) / 60
      const yStep: number = Math.abs(this.mousePos.y - y) / 60
